...
def calculate_ldc_amount(usd_value):
    return f'{float(usd_value or 0)*LDC_PER_USD:.2f}'


def calculate_usd_from_ldc(ldc_amount):
    return round(float(ldc_amount or 0)/LDC_PER_USD, 8)
...
def plan_display_ldc(plan):
    return calculate_ldc_amount(PLANS[plan]['usd_value'])
...
class H(BaseHTTPRequestHandler):
    ...
    def buy(self,params):
        ...
        body=["<div class='hero'><div><h1>LKLB LDC 充值</h1><p class='subtitle'>统一按 40 LDC 兑换 1刀额度。</p>{}</div></div>".format(login_html)]
        ...
        for amount,label in fixed_ldc:
            ...
            body.append("<p class='normal'>固定汇率 40:1</p>")
        ...
        body.append("<div class='custom-copy'><h3>自定义 LDC</h3><div class='price'>任意数量</div><p class='normal'>输入 LDC 数量，按 40:1 兑换额度。</p></div>")
    ...
    def buy_start(self,params):
        ...
        with conn() as c:
            ...
            if requested_ldc is not None:
                ...
                usd_value=calculate_usd_from_ldc(requested_ldc)
            else:
                usd_value=usd_req
                ldc_amount=calculate_ldc_amount(usd_value)
