...
// SetOpenWebUIDefault selects the user's key for Open WebUI requests.
func (h *APIKeyHandler) SetOpenWebUIDefault(c *gin.Context) {
	subject, ok := middleware2.GetAuthSubjectFromContext(c)
	if !ok {
		response.Unauthorized(c, "User not authenticated")
		return
	...
	key, err := h.apiKeyService.SetOpenWebUIDefault(c.Request.Context(), subject.UserID, keyID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
