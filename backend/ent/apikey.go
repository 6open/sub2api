...
type APIKey struct {
	...
	// Whether this key is used for the owner's Open WebUI requests
	IsOpenWebuiDefault bool `json:"is_open_webui_default,omitempty"`
	...
	for i := range columns {
		...
		case apikey.FieldIsOpenWebuiDefault:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field is_open_webui_default", values[i])
			} else if value.Valid {
				_m.IsOpenWebuiDefault = value.Bool
			}
...
func (_m *APIKey) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("is_open_webui_default=")
	builder.WriteString(fmt.Sprintf("%v", _m.IsOpenWebuiDefault))
	builder.WriteString(", ")
