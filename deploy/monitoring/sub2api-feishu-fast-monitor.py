...
def collect_probe_issues(config, state):
    ...
    for check, ok, status, latency_ms, error in results:
        ...
        # Some target-specific probes are useful for diagnostics but too noisy
        # to page on. Keep collecting their state without creating incidents.
        if not check.get("alert_enabled", True):
            continue
...
def query_slow_ai_requests(database_config):
    ...
    medium_context_tokens = max(
        1, int(database_config.get("medium_context_tokens", 50000))
    )
    medium_context_threshold_ms = max(
        threshold_ms,
        int(database_config.get("medium_context_ttft_threshold_ms", 60000)),
    )
    long_context_tokens = max(
        medium_context_tokens,
        int(database_config.get("long_context_tokens", 100000)),
    )
    long_context_threshold_ms = max(
        medium_context_threshold_ms,
        int(database_config.get("long_context_ttft_threshold_ms", 90000)),
    )
    deep_reasoning_threshold_ms = max(
        threshold_ms,
        int(database_config.get("deep_reasoning_ttft_threshold_ms", 60000)),
    )
    sql = f"""
    with recent as (
      select id,
             ...
             coalesce(nullif(model, ''), 'unknown') as model,
             first_token_ms,
             coalesce(input_tokens, 0) + coalesce(cache_read_tokens, 0) as context_tokens,
             coalesce(nullif(lower(reasoning_effort), ''), 'unknown') as reasoning_effort
      from usage_logs
      where created_at > now() - interval '{seconds} seconds'
      ...
      select *,
             greatest(
               {threshold_ms},
               case
                 when context_tokens >= {long_context_tokens} then {long_context_threshold_ms}
                 when context_tokens >= {medium_context_tokens} then {medium_context_threshold_ms}
                 else {threshold_ms}
               end,
               case
                 when reasoning_effort in ('xhigh', 'max') then {deep_reasoning_threshold_ms}
                 else {threshold_ms}
               end
             ) as effective_threshold_ms
      from recent
    )
    select coalesce(json_agg(row_to_json(t)), '[]'::json)
    from (
      select account_id,
             ...
             round(avg(context_tokens)) as avg_context_tokens,
             max(context_tokens) as max_context_tokens,
             string_agg(distinct reasoning_effort, ', ' order by reasoning_effort) as reasoning_efforts,
             min(effective_threshold_ms) as min_effective_threshold_ms,
             max(id) as max_usage_id
      from classified
      where first_token_ms >= effective_threshold_ms
...
def collect_latency_burst_issues(config, state):
    ...
    critical_minimum = max(
        1, int(latency_config.get("min_critical_slow_requests", 2))
    )
    critical_ms = max(
    ...
    critical_slow = sum(
        int(row.get("slow_requests") or 0)
        for row in rows
        if int(row.get("max_ttft_ms") or 0) >= critical_ms
    )
    if total_slow < minimum and critical_slow < critical_minimum:
        return {}
    ...
    details = [
        (
            ...
            f"上下文约 {row.get('avg_context_tokens') or 0} Token，"
            f"推理 {row.get('reasoning_efforts') or 'unknown'}"
        )
        for row in rows[:5]
    ]
    ...
    return {
        "database:ai_ttft_degraded": {
            "category": "warning",
            "title": "AI 首 Token 异常延迟",
            "body": (
                f"最近 {window_seconds} 秒有 {total_slow} 次超过动态阈值的请求；"
                + "；".join(details)
            ),
...
def main():
    ...
    if sendable or resolved:
        print(
            ...
            + json.dumps(
                {
                    "issues": [item.get("title", "") for item in sendable],
                    "resolved": [item.get("title", "") for item in resolved],
                },
                ensure_ascii=False,
            ),
            flush=True,
